...
2. Take a full-page desktop screenshot: `Use browser_task: screenshot "$REPORT_DIR/screenshots/first-impression.png"`
...
- `text-wrap: balance` or `text-pretty` on headings (check via `Use browser_task: css <heading> text-wrap`)
...
- `prefers-reduced-motion` respected (check: `Use browser_task: js "matchMedia('(prefers-reduced-motion: reduce)').matches"`)
