...
Generates a comprehensive engineering retrospective analyzing commit history, work patterns, and code quality metrics. Team-aware: identifies the user running the command, then analyzes every contributor with per-person praise and growth opportunities. Designed for a senior IC/CTO-level builder using Perplexity Computer as a force multiplier.
...
**Greptile signal (if history exists):** Read `(qstack memory) greptile-history.md` (fetched in Step 1, command 8). Filter entries within the retro time window by date. Count entries by type: `fix`, `fp`, `already-fixed`. Compute signal ratio: `(fix + already-fixed) / (fix + already-fixed + fp)`. If no entries exist in the window or the file doesn't exist, skip the Greptile metric row. Skip unparseable lines silently.
...
**Skill Usage (if analytics exist):** Read `Perplexity memory (via memory_update)` if it exists. Filter entries within the retro time window by `ts` field. Separate skill activations (no `event` field) from hook fires (`event: "hook_fire"`). Aggregate by skill name. Present as:
...
**Eureka Moments (if logged):** Read `(qstack memory) analytics/eureka.jsonl` if it exists. Filter entries within the retro time window by `ts` field. For each eureka moment, show the skill that flagged it, the branch, and a one-line summary of the insight. Present as:
...
**Note:** Only include the `greptile` field if `(qstack memory) greptile-history.md` exists and has entries within the time window. Only include the `backlog` field if `TODOS.md` exists. Only include the `test_health` field if test files were found (command 10 returns > 0). If any has no data, omit the field entirely.
...
```bash
DISCOVER_BIN=""
[ -x bin/qstack-global-discover ] && DISCOVER_BIN=bin/qstack-global-discover
[ -z "$DISCOVER_BIN" ] && [ -x (qstack skill reference) ] && DISCOVER_BIN=(qstack skill reference)
[ -z "$DISCOVER_BIN" ] && which qstack-global-discover >/dev/null 2>&1 && DISCOVER_BIN=$(which qstack-global-discover)
[ -z "$DISCOVER_BIN" ] && [ -f bin/qstack-global-discover.ts ] && DISCOVER_BIN="bun run bin/qstack-global-discover.ts"
echo "DISCOVER_BIN: $DISCOVER_BIN"
```

If no binary is found, tell the user: "Discovery script not found. Run `bun run build` in the qstack directory to compile it." and stop.

Run the discovery:
```bash
$DISCOVER_BIN --since "<window>" --format json 2>/tmp/qstack-discover-stderr
```

Read the stderr output from `/tmp/qstack-discover-stderr` for diagnostic info. Parse the JSON output from stdout.
...
- Behavioral patterns (e.g., "Codex used exclusively for myapp, Perplexity Computer for everything else")
...
- Top Work: 3 bullet points summarizing the user's major themes, inferred from
  ...
  not "feat: qstack-global-discover" + "feat: /retro global template".
...
- Perplexity Computer: N sessions across M repos — patterns observed
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t (qstack memory) retros/global-*.json 2>/dev/null | head -5
```
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
today=$(date +%Y-%m-%d)
existing=$(ls (qstack memory) retros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `(qstack memory) retros/global-${today}-${next}.json`:
...
```json
{
  ...
  "tweetable": "Week of Mar 14: 5 projects, 182 commits, 15.3k LOC | CC: 48, Codex: 8, Gemini: 3 | Focus: qstack (58%) | Streak: 52d"
}
```
