...
description: 'Post-ship documentation update. Reads all project docs, cross-references
  the
  diff, updates README/ARCHITECTURE/CONTRIBUTING to match what shipped,
...
```bash
find . -maxdepth 2 -name "*.md" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.qstack/*" -not -path "./.context/*" | sort
```
...
(adapt to whatever project you're in — these are not qstack-specific):
...
**project custom instructions / project instructions:**
...
1. Does the README's feature/capability list match what project custom instructions (or project instructions) describes?
2. Does ARCHITECTURE's component list match CONTRIBUTING's project structure description?
3. Does CHANGELOG's latest version match the VERSION file?
4. **Discoverability:** Is every documentation file reachable from README.md or project custom instructions? If
   ARCHITECTURE.md exists but neither README nor project custom instructions links to it, flag it. Every doc
...
**If GitHub:**
```bash
gh pr view --json body -q .body > /tmp/qstack-pr-body-$$.md
```

**If GitLab:**
```bash
glab mr view -F json 2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin).get('description',''))" > /tmp/qstack-pr-body-$$.md
```
...
**If GitHub:**
```bash
gh pr edit --body-file /tmp/qstack-pr-body-$$.md
```

**If GitLab:**
Read the contents of `/tmp/qstack-pr-body-$$.md` using the Read tool, then pass it to `glab mr update` using a heredoc to avoid shell metacharacter issues:
```bash
