...
**Review tiers:**
- **Eng Review (required by default):** The only review that gates shipping. Covers architecture, code quality, tests, performance. Can be disabled globally with \`memory_search/memory_update true\` (the "don't bother me" setting).
...
For Design Review: run `source <(bin/qstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
### B7. Update project custom instructions
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `review/TODOS-format.md`).
...
- Create an issue assigned to that person (use the platform detected in Step 0):
  - **If GitHub:**
    ...
    gh issue create \
      ...
      --body "Found failing on branch <current-branch>. Failure is pre-existing.\n\n**Error:**\n```\n<first 10 lines>\n```\n\n**Last modified by:** <author>\n**Noticed by:** qstack /ship on <date>" \
...
Match against these patterns (from project custom instructions):
...
1. **Read project custom instructions** — look for a `## Testing` section with test command and framework name. If found, use that as the authoritative source.
2. **If project custom instructions has no testing section, auto-detect:**
...
Before proceeding, check project custom instructions for a `## Test Coverage` section with `Minimum:` and `Target:` fields. If found, use those percentages. Otherwise use defaults: Minimum = 60%, Target = 80%.
...
Write to `(qstack memory) projects/{slug}/{user}-{branch}-ship-test-plan-{datetime}.md`:
...
# Compute project slug for (qstack memory) projects/ lookup
_PLAN_SLUG=$(git remote get-url origin 2>/dev/null | sed 's|.*[:/]\([^/]*/[^/]*\)\.git$|\1|;s|.*[:/]\([^/]*/[^/]*\)$|\1|' | tr '/' '-' | tr -cd 'a-zA-Z0-9._-') || true
_PLAN_SLUG="${_PLAN_SLUG:-$(basename "$PWD" | tr -cd 'a-zA-Z0-9._-')}"
# Search common plan file locations (project designs first, then personal/local)
for PLAN_DIR in "$HOME/.qstack/projects/$_PLAN_SLUG" "$HOME/.claude/plans" "$HOME/.codex/plans" ".qstack/plans"; do
  [ -d "$PLAN_DIR" ] || continue
...
- Review report sections (`## QSTACK REVIEW REPORT`)
...
> qstack can search learnings from your other projects on this machine to find
...
This makes the compounding visible. The user should see that qstack is getting
...
Check if the diff touches frontend files using `qstack-diff-scope`:

```bash
source <(bin/qstack-diff-scope <base> 2>/dev/null)
```
...
```bash
bin/memory_update '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
source <(bin/qstack-diff-scope <base> 2>/dev/null) || true
...
```bash
bin/qstack-specialist-stats 2>/dev/null || true
```
...
For each conditional specialist that passed scope gating, check the `qstack-specialist-stats` output above:
...
If activated, dispatch via run_subagent (foreground, not background).
...
```bash
bin/memory_update '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
dispatch via run_subagent. The subagent has fresh context — no checklist bias from the structured review. This genuine independence catches things the primary reviewer is blind to.
...
```bash
bin/memory_update '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
- Message: "QStack recommends maintaining a TODOS.md organized by skill/component, then priority (P0 at top through P4, then Completed at bottom). See TODOS-format.md for the full format. Would you like to create one?"
...
🤖 Generated with [Perplexity Computer](https://claude.com/claude-code)
```
...
2. Follow its instructions — it reads all .md files in the project, cross-references
   ...
   project custom instructions, TODOS, etc.)
...
Append to `(qstack memory) projects/$SLUG/$BRANCH-reviews.jsonl`:

```bash
echo '{"skill":"ship","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","coverage_pct":COVERAGE_PCT,"plan_items_total":PLAN_TOTAL,"plan_items_done":PLAN_DONE,"verification_result":"VERIFY_RESULT","version":"VERSION","branch":"BRANCH"}' >> (qstack memory) projects/$SLUG/$BRANCH-reviews.jsonl
```
