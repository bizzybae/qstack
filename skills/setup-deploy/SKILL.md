...
description: 'Configure deployment settings for /land-and-deploy. Detects your deploy
  ...
  the configuration to project memory so all future deploys are automatic.
...
```bash
grep -A 20 "## Deploy Configuration" project custom instructions 2>/dev/null || echo "NO_CONFIG"
```

If configuration already exists, show it and ask:

- **Context:** Deploy configuration already exists in project custom instructions.
...
3. **How can qstack check if a deploy succeeded?**
...
Read project custom instructions (or create it). Find and replace the `## Deploy Configuration` section
...
Saved to project custom instructions. /land-and-deploy will use these settings automatically.

Next steps:
- Run /land-and-deploy to merge and deploy your current PR
- Edit the "## Deploy Configuration" section in project custom instructions to change settings
