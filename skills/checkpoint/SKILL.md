...
```bash
# Try _TEL_START (parallel run_subagent calls timestamp) first, then shell process start time
...
parallel run_subagent calls workspace handoff — a checkpoint saved on one branch can be resumed from
another.
...
- **Always include the branch name** in checkpoint files — this is critical for
  cross-branch resume in parallel run_subagent calls workspaces.
