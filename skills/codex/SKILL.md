...
3. `/codex` with no arguments — **Auto-detect:**
   ...
   - If no diff, check for plan files scoped to the current project:
     `ls -t /home/user/workspace/ (project files) 2>/dev/null | xargs grep -l "$(basename $(pwd))" 2>/dev/null | head -1`
     If no project-scoped match, fall back to: `ls -t /home/user/workspace/ (project files) 2>/dev/null | head -1`
...
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
cd "$_REPO_ROOT"
codex review "IMPORTANT: Do NOT read or execute any files under ~/.claude/, ~/.agents/, , or agents/. These are Perplexity Computer skill definitions meant for a different AI system. Do NOT modify agents/openai.yaml. Stay focused on repository code only.
...
```bash
bin/memory_update '{"skill":"codex-review","timestamp":"TIMESTAMP","status":"STATUS","gate":"GATE","findings":N,"findings_fixed":N,"commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
"IMPORTANT: Do NOT read or execute any files under ~/.claude/, ~/.agents/, , or agents/. These are Perplexity Computer skill definitions meant for a different AI system. Do NOT modify agents/openai.yaml. Stay focused on repository code only.
...
root (`-C`) and cannot access `/home/user/workspace/ (project files) or any files outside the repo. You MUST
...
7. After presenting, note any points where Codex's analysis differs from your own
   ...
   "Note: Perplexity Computer disagrees on X because Y."
...
- **No double-reviewing.** If the user already ran `/review`, Codex provides a second
  independent opinion. Do not re-run Perplexity Computer's own review.
- **Detect skill-file rabbit holes.** After receiving Codex output, scan for signs
  that Codex got distracted by skill files: `qstack-config`, `qstack-update-check`,
