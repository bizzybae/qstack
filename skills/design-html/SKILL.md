...
_CEO=$(ls -t (qstack memory) projects/$SLUG/ceo-plans/*.md 2>/dev/null | head -1)
[ -n "$_CEO" ] && echo "CEO_PLAN: $_CEO" || echo "NO_CEO_PLAN"
```

```bash
setopt +o nomatch 2>/dev/null || true
_APPROVED=$(ls -t (qstack memory) projects/$SLUG/designs/*/approved.json 2>/dev/null | head -1)
[ -n "$_APPROVED" ] && echo "APPROVED: $_APPROVED" || echo "NO_APPROVED"
```

```bash
setopt +o nomatch 2>/dev/null || true
_VARIANTS=$(ls -t (qstack memory) projects/$SLUG/designs/*/variant-*.png 2>/dev/null | head -1)
[ -n "$_VARIANTS" ] && echo "VARIANTS: $_VARIANTS" || echo "NO_VARIANTS"
```

```bash
setopt +o nomatch 2>/dev/null || true
_FINALIZED=$(ls -t (qstack memory) projects/$SLUG/designs/*/finalized.html 2>/dev/null | head -1)
...
[ -n "$_ROOT" ] && [ -f "(qstack skill reference) ] && _PRETEXT_VENDOR="(qstack skill reference)
...
`(qstack memory) projects/$SLUG/designs/<screen-name>-YYYYMMDD/finalized.html`

For framework output, save to:
`(qstack memory) projects/$SLUG/designs/<screen-name>-YYYYMMDD/finalized.[tsx|svelte|vue]`
...
```bash
Use browser_task to navigate to "file://<path-to-finalized.html>"
Use browser_task: screenshot /tmp/qstack-verify-mobile.png --width 375
Use browser_task: screenshot /tmp/qstack-verify-tablet.png --width 768
Use browser_task: screenshot /tmp/qstack-verify-desktop.png --width 1440
```
