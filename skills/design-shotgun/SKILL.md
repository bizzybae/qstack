...
ls (qstack memory) projects/$SLUG/*office-hours* 2>/dev/null | head -5
```
...
```bash
setopt +o nomatch 2>/dev/null || true
_TASTE=$(find (qstack memory) projects/$SLUG/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -10)
```
...
```bash
Use browser_task: screenshot "$_DESIGN_DIR/current.png"
```
...
**Why /tmp/ then cp?** In observed sessions, `$D generate --output (qstack memory) ...`
...
1. **Never save to `.context/`, `docs/designs/`, or `/tmp/`.** All design artifacts go
   to `(qstack memory) projects/$SLUG/designs/`. This is enforced. See DESIGN_SETUP above.
