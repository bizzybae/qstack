...
```bash
# Check for persisted deploy config in project custom instructions
DEPLOY_CONFIG=$(grep -A 20 "## Deploy Configuration" project custom instructions 2>/dev/null || echo "NO_CONFIG")
echo "$DEPLOY_CONFIG"
...
If `PERSISTED_PLATFORM` and `PERSISTED_URL` were found in project custom instructions, use them directly
...
and any persisted config from project custom instructions.
...
1. **project custom instructions persisted config:** Check for a staging URL in the Deploy Configuration section:
```bash
grep -i "staging" project custom instructions 2>/dev/null | head -3
```
...
```bash
mkdir -p (qstack memory) projects/$SLUG
CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' project custom instructions 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
WORKFLOW_HASH=$(find .github/workflows -maxdepth 1 \( -name '*deploy*' -o -name '*cd*' \) 2>/dev/null | xargs cat 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
echo "${CURRENT_HASH}-${WORKFLOW_HASH}" > (qstack memory) projects/$SLUG/land-deploy-confirmed
```
Continue to Step 2.
...
**If C:** **STOP.** "Running `/setup-deploy` will walk through your deploy platform, production URL, and health checks in detail. It saves everything to project custom instructions so I'll know exactly what to do next time. Run `/land-and-deploy` again when that's done."
...
```bash
cat review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
Read project custom instructions to find the project's test command. If not specified, use `bun test`.
...
ls -t /home/user/workspace/.qstack/evals/*-e2e-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -20
```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t /home/user/workspace/.qstack/evals/*-llm-judge-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -5
```
...
```bash
git diff --name-only $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main)...HEAD -- README.md CHANGELOG.md ARCHITECTURE.md CONTRIBUTING.md project custom instructions VERSION
```
...
Then run `qstack-diff-scope` to classify the changes:

```bash
eval $(bin/qstack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
...
If staging was detected in Step 1.5c (or from project custom instructions deploy config), and the changes
...
If a deploy status command was configured in project custom instructions (e.g., `fly status --app myapp`), use it instead of or in addition to GitHub Actions polling.
...
If project custom instructions has a custom deploy status command in the "Custom deploy hooks" section, run that command and check its exit code.
...
| SCOPE_CONFIG only | Smoke: `Use browser_task: goto` + verify 200 status |
...
```bash
Use browser_task: text
```

Verify the page has content (not blank, not a generic error page).

```bash
Use screenshot_page to capture the page -a -o ".qstack/deploy-reports/post-deploy.png"
```
...
Save report to `.qstack/deploy-reports/{date}-pr{number}-deploy.md`.
