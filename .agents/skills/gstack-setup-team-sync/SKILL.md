...
```bash
$GSTACK_ROOT/bin/gstack-sync status 2>&1
```
...
```bash
$GSTACK_ROOT/bin/gstack-sync setup 2>&1
```
...
```bash
$GSTACK_ROOT/bin/gstack-sync test 2>&1
```
...
```bash
$GSTACK_ROOT/bin/gstack-config get sync_enabled 2>/dev/null
$GSTACK_ROOT/bin/gstack-config get sync_transcripts 2>/dev/null
```
...
- If they say yes:
  ```bash
  $GSTACK_ROOT/bin/gstack-config set sync_enabled true
  $GSTACK_ROOT/bin/gstack-config set sync_transcripts true
  ```
